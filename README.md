# 🎓 THIẾT KẾ VÀ TRIỂN KHAI HỆ THỐNG QUẢN LÝ KHOÁ LUẬN VÀ ĐỒ ÁN TỐT NGHIỆP TÍCH HỢP CHỨC NĂNG KIỂM TRA TƯƠNG ĐỒNG VĂN BẢN
> **DESIGN AND IMPLEMENT THE THESIS AND GRADUATION PROJECT MANAGEMENT SYSTEM INTEGRATED WITH TEXT SIMILARITY CHECKING**

* **Tác giả:** Nguyễn Hoàng Phúc, Văn Thị Thiên Trang
* **Đơn vị:** Khoa Công nghệ thông tin, Trường Đại học Kinh tế - Tài chính Thành phố Hồ Chí Minh, Việt Nam (UEF)
* **Email:** phucnh322@uef.edu.vn, trangvtt@uef.edu.vn
...
- `Approved` → Tài liệu được phê duyệt chính thức bởi Admin và xuất bản công khai lên Thư viện số cho sinh viên tra cứu.
...
- **Quản lý người dùng (User & Role Management):** Quản lý, kích hoạt tài khoản Sinh viên, Giảng viên.
...
| **ApprovedAt** | `DateTime?` | Nullable | Ngày phê duyệt và xuất bản thư viện số |
...
| **Major** | `string?` | Nullable | Chuyên ngành học (Ví dụ: `ai`, `security`, `software-engineering`) |
| **Subject** | `string?` | Nullable | Tên môn học học thuật tương ứng |
| **SubjectCode** | `string?` | Nullable | Mã môn học học thuật |
| **Category** | `string` | Not Null, Default: `Project` | Phân loại học thuật tương ứng: `Project`, `Topic`, `Thesis` |
| **Batch** | `int` | Not Null, Default: `1` | Khóa/Đợt thực hiện học thuật |
...
##### 4. Bảng `ThesisReviews` (Đánh giá & Chấm điểm)
Lưu trữ thông tin điểm số và nhận xét phê duyệt của giảng viên chấm/phản biện.
...
| **CloudinaryStatus** | `string` | Not Null, Default: `None` | Trạng thái tải ảnh lên Cloudinary: `None`, `Pending`, `Uploaded`, `Failed` |
...
##### 12. Bảng `DriveFileRecords` (Tệp tin đồng bộ Google Drive)
Theo dõi thông tin và trạng thái đồng bộ hóa các tệp tin từ Google Drive (thư mục Temporary_PDF) do công việc nền Hangfire thực hiện định kỳ.

| Tên cột | Kiểu dữ liệu | Ràng buộc | Mô tả |
| :--- | :--- | :--- | :--- |
| **Id** | `int` | PK, Identity | Mã định danh duy nhất của bản ghi tệp tin |
| **DriveFileId** | `string` | Not Null | Mã định danh của tệp tin trên Google Drive |
| **FileName** | `string` | Not Null | Tên tệp tin gốc trên Drive |
| **MimeType** | `string` | Not Null | Kiểu tệp tin MIME (Ví dụ: `application/pdf`) |
| **FileSize** | `long?` | Nullable | Dung lượng tệp tin tính theo byte |
| **WebViewLink** | `string` | Not Null | Đường dẫn xem tệp trực tiếp trên Google Drive |
| **WebContentLink** | `string` | Not Null | Đường dẫn tải xuống tệp trực tiếp |
| **SourceFolder** | `string` | Not Null | Thư mục nguồn đồng bộ trên Drive (Ví dụ: `Temporary_PDF`) |
| **Category** | `string` | Not Null, Default: `Project` | Phân loại học thuật tương ứng: `Project`, `Topic`, `Thesis` |
| **DriveCreatedAt** | `DateTime?` | Nullable | Thời gian tạo tệp tin trên Google Drive |
| **DriveModifiedAt** | `DateTime?` | Nullable | Thời gian cập nhật gần nhất trên Google Drive |
| **SyncedAt** | `DateTime` | Not Null, Default: `UtcNow` | Thời gian đồng bộ bản ghi về hệ thống |
| **LastCheckedAt** | `DateTime` | Not Null, Default: `UtcNow` | Thời điểm gần nhất kiểm tra sự tồn tại của tệp trên Drive |
| **IsActive** | `bool` | Not Null, Default: `true` | Trạng thái tệp tin còn tồn tại trên Drive hay không |
| **RelativePath** | `string` | Not Null | Đường dẫn tương đối trên Drive |
| **Major** | `string` | Not Null | Chuyên ngành học |
| **MajorKey** | `string` | Not Null | Khóa chuyên ngành |
| **Subject** | `string` | Not Null | Tên môn học |
| **SubjectCode** | `string` | Not Null | Mã môn học |
| **StudentUid** | `string` | Not Null | Mã số sinh viên của người thực hiện |
| **ProjectName** | `string` | Not Null | Tên đề tài khóa luận/đồ án |
| **LocalPdfPath** | `string` | Not Null | Đường dẫn tệp tin PDF lưu cục bộ sau chuyển đổi |
...
*   **Trách nhiệm chính:**
    *   Quản lý các thực thể chính của hệ thống như Sinh viên (`Student`), Giảng viên (`Advisor`), Đề tài (`Theses`).
