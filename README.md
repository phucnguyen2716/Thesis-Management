...
erDiagram
    ...
    THESES ||--o{ THESIS_SUBMISSIONS : "có các bản nộp"
...
| **Status** | `string` | Not Null | Trạng thái: `Pending`, `InProgress`, `Submitted`, `UnderReview`, `Approved`, `Rejected`, `Revision` |
| **FilePath** | `string?` | Nullable | Đường dẫn lưu trữ file báo cáo chính thức |
| **RejectReason** | `string?` | Nullable | Lý do từ chối đề tài (nếu trạng thái là `Rejected`) |
| **CreatedAt** | `DateTime` | Not Null, Default: `UtcNow` | Ngày đăng ký đề tài |
| **UpdatedAt** | `DateTime?` | Nullable | Ngày cập nhật thông tin gần nhất |
| **SubmittedAt** | `DateTime?` | Nullable | Ngày nộp báo cáo chính thức |
...
##### 3. Bảng `ThesisSubmissions` (Các bản nộp tiến độ / Bản thảo)
Lưu trữ lịch sử các phiên bản file báo cáo tiến độ và bản thảo chính thức do sinh viên nộp lên.
...
| **Notes** | `string?` | Nullable | Ghi chú của sinh viên gửi tới giảng viên khi nộp bài |
...
Phân hệ **AI Chatbot** đóng vai trò là một trợ lý nghiên cứu học thuật đắc lực hỗ trợ sinh viên khai thác và tìm kiếm thông tin trong **Thư viện số (Digital Library)**, hoàn toàn tách biệt với phân hệ nộp khóa luận. Chatbot được xây dựng dựa trên kiến trúc **Double-Guardrail Sandwich** bảo mật kết hợp thuật toán xếp hạng văn bản chuyên sâu **BM25**.
