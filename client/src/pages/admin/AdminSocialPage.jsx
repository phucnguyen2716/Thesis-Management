...
const AdminSocialPage = () => {
  ...
  const load = () => setPosts(getAllSocialPosts());
  useEffect(() => {
    ensureContentSeed();
    load();
  ...
  const toggleChannel = ch => {
    setForm(prev => {
      ...
      return {
        ...prev,
        channels: chs.includes(ch) ? chs.filter(c => c !== ch) : [...chs, ch],
      };
    });
  };
    ...
    <div className="max-w-6xl space-y-5">
      <div className="flex flex-col sm:flex-row sm:justify-between gap-3">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Tin/bài đăng hiển thị trên portal sinh viên (/news) · kênh portal, Facebook, LinkedIn…
          </p>
        </div>
        <button
          type="button"
          ...
          >
            ...
            <div className="p-4 flex-1 flex flex-col">
              <div className="flex justify-between gap-2 mb-2">
                ...
                <span
                  className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                    p.published ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400'
                  }`}
                >
              ...
              <p className="text-[10px] text-slate-500 mt-2">
                {p.date} · {(p.channels || []).join(', ')}
              </p>
              <div className="flex gap-2 mt-3">
                <button
                  type="button"
                  onClick={() => openEdit(p)}
                  className="flex-1 py-1.5 rounded-lg bg-slate-700 text-xs font-bold text-white"
                >
                  Sửa
                </button>
                <button
                  type="button"
                  onClick={() => {
                    if (window.confirm('Xóa bài đăng?')) {
                      deleteSocialPost(p.id);
                      load();
                    }
                  }}
                  className="px-3 py-1.5 rounded-lg bg-red-900/40 text-xs font-bold text-red-300"
                >
                  Xóa
                </button>
              </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 overflow-y-auto">
          <form
            onSubmit={handleSave}
            className="w-full max-w-lg rounded-xl bg-slate-900 border border-slate-700 p-5 space-y-3 my-4"
          >
            <h2 className="text-lg font-bold text-white">
              {modal.mode === 'create' ? 'Đăng bài mới' : 'Sửa bài đăng'}
            </h2>
