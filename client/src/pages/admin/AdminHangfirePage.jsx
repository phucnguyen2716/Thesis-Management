...
const AdminHangfirePage = () => {
      ...
      <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
        <div className="text-center max-w-md mx-auto px-6">
          ...
          <p className="text-xs text-slate-400 leading-relaxed mb-8">
            Tài khoản <span className="font-semibold text-slate-600">{user?.fullName || 'của bạn'}</span> ({user?.role || 'Unknown'}) không có quyền truy cập vào khu vực quản trị hệ thống này. Vui lòng liên hệ quản trị viên nếu bạn cần hỗ trợ.
          </p>
          ...
          {/* Server Info */}
          <div className="mt-10 p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
            <div className="flex items-center justify-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-widest">
              <Server size={11} />
              <span>Hangfire Job Processing Server</span>
            </div>
            <p className="text-[10px] text-slate-400 mt-1.5">
              Quản lý background jobs, recurring tasks và job queue monitoring.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      {/* Header */}
      <div className="h-14 px-6 flex items-center justify-between border-b border-slate-100 bg-white shrink-0">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 border border-emerald-200 rounded-full text-[10px] font-bold text-emerald-700">
