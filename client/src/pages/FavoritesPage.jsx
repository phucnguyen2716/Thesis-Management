...
const FavoritesPage = () => {
  ...
  return (
    <div className="min-h-screen bg-surface-bright pb-20 overflow-hidden">
      ...
      <section className="relative h-[550px] w-full bg-[#0a0a0a] overflow-hidden group">
        {favoriteItems.map((item, idx) => (
          ...
          >
            ...
            {/* Content Overlay */}
            <div className="absolute inset-0 flex items-center px-8 md:px-20 pb-20">
              <div className={`max-w-2xl transition-all duration-700 delay-200 ${
                ...
                <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary/20 backdrop-blur-xl rounded-lg text-primary font-black text-[9px] uppercase tracking-[0.3em] mb-6 border border-primary/30">
                ...
                <div className="flex flex-wrap items-center gap-6 mb-8">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/5 backdrop-blur-xl rounded-xl flex items-center justify-center text-white border border-white/10">
                      <span className="material-symbols-outlined text-xl">person</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[8px] font-black text-white/30 uppercase tracking-widest leading-none mb-1">Sinh viên</span>
                      <span className="text-xs font-bold text-white/90">{item.student}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/5 backdrop-blur-xl rounded-xl flex items-center justify-center text-white border border-white/10">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[8px] font-black text-white/30 uppercase tracking-widest leading-none mb-1">Hướng dẫn</span>
                      <span className="text-xs font-bold text-white/90">{item.advisor}</span>
                    </div>
                  </div>
                </div>
                <div className="flex gap-3">
                  <button 
                    onClick={() => navigate(`/theses/${item.id}`)}
                    className="px-8 py-3.5 bg-primary text-on-primary rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-white hover:text-primary transition-all shadow-xl shadow-primary/20 flex items-center gap-2"
                  >
                    XEM CHI TIẾT
                    <span className="material-symbols-outlined text-base">arrow_right_alt</span>
                  </button>
                  <button className="w-12 h-12 bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl flex items-center justify-center text-white hover:bg-white/10 transition-all">
        ...
        <div className="absolute bottom-10 right-8 md:right-20 z-30 flex items-end gap-3">
           {favoriteItems.map((item, idx) => (
             <button 
               key={item.id}
        ...
        <div className="absolute right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-2">
           {favoriteItems.map(item => (
             <div 
               key={item.id}
               className={`w-0.5 transition-all duration-500 rounded-full ${selectedId === item.id ? 'h-8 bg-primary' : 'h-2 bg-white/10'}`}
             />
           ))}
        </div>
      </section>
      ...
      <section className="pt-20 px-8 md:px-16 lg:px-24 max-w-[1300px] mx-auto w-full">
        <div className="flex items-center gap-4 mb-12 animate-fade-in">
          <div className="w-1.5 h-8 bg-primary rounded-full"></div>
          <h2 className="text-3xl font-black text-on-surface tracking-tight uppercase">Tất cả yêu thích</h2>
          ...
          {favoriteItems.map((item, idx) => (
            ...
            >
              ...
              <div className="p-6 md:p-8 flex-1 flex flex-col justify-center">
                ...
                <h3 className="text-xl md:text-2xl font-black text-on-surface mb-3 line-clamp-2 leading-tight group-hover:text-primary transition-colors tracking-tight">
                  {item.title}
                </h3>
