...
const WordPlayground = () => {
  ...
  return (
    <div className="flex flex-col h-full min-h-[calc(100vh-6.5rem)] bg-[#f3f2f1] rounded-2xl border border-outline-variant overflow-visible md:overflow-hidden shadow-lg relative">
      {/* Header thanh công cụ sticky */}
      <div className="sticky top-0 z-30 flex flex-col shrink-0 shadow-md">
        {/* Word-like title bar */}
        <div className="bg-[#185abd] text-white px-3 py-1 flex flex-col md:flex-row md:items-center justify-between gap-1">
        ...
        {/* Ribbon */}
        <div className="bg-white border-b border-slate-200 px-2 py-0.5 shrink-0 overflow-x-auto">
          <div className="flex flex-row gap-2.5 min-w-max items-center">
            {TOOLBAR_GROUPS.map(group => (
              <div key={group.label} className="flex flex-col border-r border-slate-100 pr-2 last:border-0">
                <span className="text-[7.5px] text-slate-400 uppercase font-black tracking-widest mb-0.5">{group.label}</span>
                <div className="flex gap-0.5">
                  {group.items.map(item => (
                    <button
                      ...
                      className="w-6.5 h-6.5 rounded hover:bg-slate-100 flex items-center justify-center text-slate-700 active:bg-slate-200 transition-colors"
                    >
            ...
            <div className="flex flex-col border-r border-slate-100 pr-2">
              <span className="text-[7.5px] text-slate-400 uppercase font-black tracking-widest mb-0.5">Đánh số nâng cao</span>
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`px-2 h-6.5 rounded flex items-center gap-1 text-[8.5px] font-bold transition-all ${
                    isMultilevelActive 
                      ? 'bg-primary text-white shadow-sm' 
                      : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}
                >
                  <span className="material-symbols-outlined text-xs">format_list_numbered_rtl</span>
                  Số nhiều cấp
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col border-r border-slate-100 pr-2">
              <span className="text-[7.5px] text-slate-400 uppercase font-black tracking-widest mb-0.5">Chọn chương luyện tập</span>
              <div className="flex items-center gap-1 h-6.5">
                {[
                  ...
                  <button
                    ...
                    className={`px-2 h-6.5 rounded text-[8.5px] font-black uppercase tracking-wider transition-all duration-200 ${
            ...
            {/* Áp dụng Templates */}
            <div className="flex flex-col border-r border-slate-100 pr-2">
              <span className="text-[7.5px] text-slate-400 uppercase font-black tracking-widest mb-0.5">Mẫu cấu trúc</span>
                ...
                {templates
                  ...
                  .map(t => (
                    <button
                      ...
                      className={`px-2 h-6.5 rounded text-[8.5px] font-black uppercase tracking-wider transition-colors ${
            ...
            <div className="flex flex-col">
              <span className="text-[7.5px] text-slate-400 uppercase font-black tracking-widest mb-0.5">Thu phóng</span>
              <div className="flex items-center gap-1 h-6.5">
                <button
                  type="button"
                  onClick={() => setZoom(z => Math.max(70, z - 10))}
                  className="w-6 h-6 rounded hover:bg-slate-100 text-xs font-bold flex items-center justify-center border border-slate-200"
                >
                  −
                </button>
                <span className="text-[9.5px] font-black w-8 text-center text-slate-700">{zoom}%</span>
                <button
                  type="button"
      ...
      {templates.find(t => t.id === activeTemplateId) && (
        <div className="bg-teal-50/60 border-b border-teal-100 px-3 py-0.5 flex flex-wrap items-center justify-between gap-1 shrink-0 text-[10px]">
          <div className="flex items-center gap-1.5">
            <span className="material-symbols-outlined text-teal-800 text-[13px]">info</span>
            ...
            <span className="text-[8px] text-slate-400">|</span>
            <span className="text-slate-500">
              Đề mục bắt buộc: <code className="bg-white px-1.5 py-0.2 rounded border border-teal-100 text-[9.5px] font-bold text-teal-800">{templates.find(t => t.id === activeTemplateId).requiredSections?.join(', ')}</code>
            </span>
          </div>
          {latestSub && (
            <div className="flex items-center gap-1.5">
              <span className="text-slate-400 text-[9px]">Trạng thái bài nộp:</span>
              {latestSub.teacherGraded ? (
                <span className="px-1.5 py-0.2 bg-emerald-100 text-emerald-800 rounded-full font-bold text-[9px]">
                  Giảng viên chấm: {latestSub.teacherGrade}/10
                </span>
              ) : (
                <span className="px-1.5 py-0.2 bg-emerald-100 text-emerald-800 rounded-full font-bold text-[9px]">
                  AI Đánh giá hoàn tất: {latestSub.aiScore}/10
                </span>
              )}
            </div>
          )}
      ...
      <div 
        ref={containerRef}
        className="flex-1 overflow-auto p-2 md:p-4 bg-[#e8e6e3] relative flex justify-center items-start"
      >
        <div
          style={{
